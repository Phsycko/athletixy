...
export default function NutriologoPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white mb-2">Nutriólogo</h1>
        <p className="text-gray-400">Consultas y seguimiento nutricional</p>
      </div>
      ...
      <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-6 shadow-xl">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1">
            <h2 className="text-2xl font-bold text-white mb-1">{nutriologo.nombre}</h2>
            <p className="text-white/80 mb-3">{nutriologo.especialidad}</p>
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center gap-2 text-white/90">
                <Star className="w-4 h-4 fill-yellow-300 text-yellow-300" />
                <span>{nutriologo.calificacion} / 5.0</span>
              </div>
              <div className="text-white/90">{nutriologo.experiencia} de experiencia</div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white/10 border border-gray-600/30 rounded-lg p-5 mb-6">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="text-white font-semibold">{proximaConsulta.fecha}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-400">Hora</span>
                <span className="text-white font-semibold">{proximaConsulta.hora}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-400">Duración</span>
                <span className="text-white font-semibold">{proximaConsulta.duracion}</span>
              </div>
            </div>
          </div>
          <div className="flex gap-3">
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-white mb-6">Recomendaciones Actuales</h2>
          <div className="space-y-3">
            {recomendaciones.map((recomendacion, index) => (
              <div
                key={index}
                className="flex items-start gap-3 p-4 bg-dark-900 rounded-lg border border-dark-600"
              >
      ...
      <div className="bg-dark-800 border border-dark-700 rounded-xl p-6">
        <h2 className="text-lg font-semibold text-white mb-6">Historial de Consultas</h2>
          ...
          {consultasAnteriores.map((consulta, index) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="text-white font-semibold mb-1">{consulta.tipo}</h3>
              ...
              <p className="text-gray-400 text-sm">{consulta.notas}</p>
            </div>
          ))}
        </div>
      </div>
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white font-medium text-sm mb-1 truncate">{doc.nombre}</p>
      ...
      <div className="bg-dark-800 border border-dark-700 rounded-xl p-6">
        <h2 className="text-lg font-semibold text-white mb-4">Agendar Nueva Consulta</h2>
